...
export default function UserHeader({user, userId}) {
            ...
            <div>
                {!(user._id === userId) ?
                    ...
                    <Link to={`/user/${userId}/edit`}>
                        <button onClick={clickHandler} className="px-2 py-1 lg:px-4 lg:py-2 text-sm font-medium text-white bg-blue-600 border border-blue-400 rounded-lg hover:bg-blue-700 hover:text-white focus:z-10 focus:ring-1 focus:ring-blue-600 focus:text-white-600  ">Edit Profile</button>
                    </Link>
                }
            </div>
        </div>
